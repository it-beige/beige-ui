...
:::demo ☑️ 勾选功能，传入 checkable

```vue
<template>
  <BgTree :data="data" checkable></BgTree>
</template>
<script setup>
import { ref } from 'vue'
const data = ref([
...
```vue
<template>
  <BgTree :data="data" operable></BgTree>
</template>
<script lang="ts">
...
```vue
<template>
  <BgTree :data="data" @lazy-load="lazyLoad"></BgTree>
</template>
<script>
import { defineComponent, ref } from 'vue'
...
:::demo 通过`dragdrop`属性配置节点的拖拽功能。

```vue
<template>
  <h6><p>默认行为</p></h6>
  <BgTree :data="data" dragdrop></BgTree>
...
```vue
<template>
  <BgTree :data="data" :height="300"></BgTree>
</template>
<script lang="ts">
import { defineComponent, ref } from 'vue'
